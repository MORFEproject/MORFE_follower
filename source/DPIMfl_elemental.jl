...
function B3_fl!(Fe::Matrix{Float64},KeUU::Matrix{Float64},
                Xe::Matrix{Float64},Ue::Matrix{Float64},press)
...
for (w,a) in qr
  ...
  FU=D*(Xe+Ue)
  TD=[-FU[2] FU[1]]'                 # inward normal pressure
  NL=[.5*a*(a-1) .5*a*(1+a) 1-a^2]   # shape functions
  N=[NL[1] 0 NL[2] 0 NL[3] 0;
     0 NL[1] 0 NL[2] 0 NL[3]]

  Fe[:]+=N'*TD*w*press
  KeUU[:,:]-=N'*[0 -D[1] 0 -D[2] 0 -D[3]; 
                 D[1] 0  D[2] 0  D[3] 0]*w*press  
